(function (Scratch) {
	...
	if (!Scratch.extensions.unsandboxed) {
		throw new Error("Cat Toolbox extension needs to be run unsandboxed");
	}
	const TURBO_MODE = "Turbo-Modus";
	const INTERPOLATION = "Interpolation";
	const REMOVE_FENCING = "Einzäunung entfernen";
	const REMOVE_MISC_LIMITS = "Verschd. Limits entfernen";
	const HIGH_QUALITY_PEN = "HD-Stift";
	const FRAMERATE = "FPS";
	const emitChanged = (what) => {
		Scratch.vm.runtime.startHats("cattoolbox_whenChange", { WHAT: what });
	};
			...
			return {
				...
				blocks: [{
					...
					},
					{
						opcode: 'dummyLabel',
						blockType: Scratch.BlockType.LABEL,
						text: 'Laufzeit-Optionen'
					},
					{
						opcode: "getEnabled",
						text: "[thing] aktiviert?",
						blockType: Scratch.BlockType.BOOLEAN,
						arguments: {
							thing: {
								type: Scratch.ArgumentType.STRING,
								defaultValue: "Turbo-Modus",
								menu: "thing"
							}
						}
					},
				...
				menus: {
					...
					thing: {
						acceptReporters: true,
						items: [
							{ text: "Turbo-Modus", value: "Turbo-Modus" },
							{ text: "Interpolation", value: "Interpolation" },
							{ text: "Einzäunung entfernen", value: "Einzäunung entfernen" },
							{ text: "Verschd. Limits entfernen", value: "Verschd. Limits entfernen" },
							{ text: "HD-Stift", value: "HD-Stift" }
						]
					},
					enabled: {
						acceptReporters: true,
	...
	/* Trennung */
		...
		setEnabled({ thing, enabled }) {
			...
			if (thing === "Turbo-Modus") {
				...
				Scratch.vm.setRuntimeOptions({ fencing: !enabled });
			} else if (thing === "Verschd. Limits entfernen") {
				Scratch.vm.setRuntimeOptions({ miscLimits: !enabled });
			} else if (thing === "HD-Stift") {
